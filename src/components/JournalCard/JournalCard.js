...
export const JournalCard = ({ fullDate, title, excerpt, link }) => {
    ...
    return (
        <Link href={link} className={styles.JournalCard}>
            ...
            <div className="h-full w-full flex flex-col justify-between gap-4 lg:gap-8">
                ...
                {excerpt?.length > 0 ? (
                    <p className="font-display text-base">{asText(excerpt)}</p>
                ) : (
                    <p className="font-display text-base">Click to read more</p>
                )}
