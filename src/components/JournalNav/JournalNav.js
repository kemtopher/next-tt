...
export const JournalNav = ({ prev, next, className }) => {
    ...
    return (
        <div
            className={`w-full flex justify-between gap-4 p-4 border-t pt-8 ${className}`}
        >
            {/* Previous (left) */}
            {prev ? (
                <Link
                ...
                >
                    <p className="pb-2 text-sm font-medium italic text-black">
                        previous:
                    </p>
                    <p className="truncate">
                        {truncateContent(prev.title, 72)} ...
                    </p>
                </Link>
            ) : null}
