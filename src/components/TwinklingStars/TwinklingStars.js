...
export const TwinklingStars = ({ count = 20, className = '' }) => {
  ...
  return (
    <div className={`dots-layer pointer-events-none fixed inset-0 z-10 ${className}`}>
      {Array.from({ length: count }).map((_, i) => <Dot key={i} />)}
      <style jsx global>{`
        @keyframes dot-pop {
          0%   { transform: scale(0);   opacity: 0; }
          20%  { transform: scale(1.8); opacity: 0.9; }
          35%  { transform: scale(1);   opacity: 1; }
          80%  { transform: scale(2);   opacity: 1; }
          100% { transform: scale(1);   opacity: 0; }
        }
        .dots-layer > span {
          animation: dot-pop var(--dur, 6s) cubic-bezier(.33,0,.2,1) var(--delay, 0s) infinite;
